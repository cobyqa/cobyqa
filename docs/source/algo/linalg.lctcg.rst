...
residual becomes small. More precisely, for some feasible :math:`x \in \R^n`,
we let
...
It builds the QR factorization of the matrix whose columns are the gradients of
the active constraints (namely the linear equality constraints and the linear
inequality constraints indexed by :math:`\mathcal{I}`). We denote
...
orthogonal. Powell showed in :cite:`lctcg-Powell_2015` that the solution of
problem :eq:`init-search` is given by
...
iterations should terminate. Thus, the iterations are also stopped if the
reduction provided by the current search direction is tiny compared to the
reduction so far, that is if
