...
However, such a problem can clearly be reformulated by introducing a slack variable :math:`y \in \R^{m_1}` as
...
In doing so, we observe that problem :eq:`cpqp-slack` is almost of the right form to be solved approximately by the algorithm `lctcg`, as detailed in the :ref:`presentation of the method <linalg.lctcg>`.
Only the trust-region constraint differs, and it not applied to all variables of the reformulated problem.
...
Thus, method described below is a variation of the `lctcg` algorithm.
...
#. Set :math:`g_x^0 = C^{\T} (C x^0 - d)`, :math:`d_x^0 = -\Pi_x(g_x^0)`, :math:`d_y^0 = -\Pi_y(y^0)`, the working set :math:`\mathcal{I} \subseteq \mathcal{J}(x^0, y^0)`, and :math:`k = 0`.
