...
def tangential_byrd_omojokun(grad, hess_prod, xl, xu, delta, debug, **kwargs):
    ...
    .. math::
        ...
        \min_{s \in \R^n} \quad \transpose{g} s + \frac{1}{2} \transpose{s} H s \quad \text{s.t.} \quad
        \left\{ \begin{array}{l}
            \xl \le s \le \xu,\\
    ...
    if kwargs.get('improve', True) and boundary_reached:
        ...
        while np.count_nonzero(free_bd) > 0:
            ...
            sd[~free_bd] = 0.0
            if grad_sd >= -1e-8 * reduct or np.any(grad_sd >= -np.finfo(float).tiny * np.abs(sd[free_bd])):
                break
            sd[free_bd] /= -grad_sd
...
def constrained_tangential_byrd_omojokun(grad, hess_prod, xl, xu, aub, bub, aeq, delta, debug, **kwargs):
    ...
    if kwargs.get('improve', True) and boundary_reached and n_act < n:
        ...
        while n_act < n:
            ...
            if grad_sd >= -1e-8 * reduct or np.any(grad_sd >= -np.finfo(float).tiny * np.abs(sd)):
                break
            sd /= -grad_sd
...
def normal_byrd_omojokun(aub, bub, aeq, beq, xl, xu, delta, debug, **kwargs):
    ...
    .. math::
        ...
        \min_{s \in \R^n} \quad \frac{1}{2} \big( \lVert \max \{ \aub s - \bub, 0 \} \rVert^2 + \lVert \aeq s - \beq \rVert^2 \big) \quad \text{s.t.} \quad
        \left\{ \begin{array}{l}
            \xl \le s \le \xu,\\
            \lVert s \rVert \le \Delta,
        \end{array} \right.
