...
class TestMinimizeBase(ABC):
    ...
    @staticmethod
    def arwhead(x):
        return np.sum((x[:-1] ** 2.0 + x[-1] ** 2.0) ** 2.0
                      - 4.0 * x[:-1] + 3.0)
...
class TestMinimizeUnconstrained(TestMinimizeBase):
    ...
    @pytest.mark.parametrize(
        "fun", ["arwhead", "rosen", "rothyp", "sphere", "sumsqu", "trid"]
    )
...
class TestMinimizeBoundConstrained(TestMinimizeBase):
    ...
    def test_target(self, fun, n, x0, xl, xu, x_best, fun_best):
        res = minimize(
            ...
            x0=x0,
            bounds=Bounds(xl, xu),
            options={"target": fun_best + 1.0, "debug": True},
        )
        self.assert_result(res, n, x_best, fun_best + 1.0, 1, 0.0)
    ...
    def test_restricted(
        ...
        res = minimize(
            fun=getattr(self, fun),
            x0=x0,
            bounds=Bounds(xl_restricted, xu_restricted),
            options={"debug": True},
        )
        self.assert_result(res, n, x_best_restricted, fun_best_restricted, 0,
                           0.0)
...
class TestMinimizeLinearInequalityConstrained(TestMinimizeBase):
    ...
    def test_simple(self, fun, n, x0, xl, xu, aub, bub, x_best, fun_best):
        ...
        self.assert_result(res, n, x_best, fun_best, 0,
    ...
    def test_target(self, fun, n, x0, xl, xu, aub, bub, x_best, fun_best):
        res = minimize(
            ...
            x0=x0,
            constraints=LinearConstraint(aub, -np.inf, bub),
            options={"debug": True, "target": fun_best + 1.0},
        )
        self.assert_result(
            res, n, x_best, fun_best + 1.0, 1, np.sqrt(np.finfo(float).eps)
        )
...
class TestMinimizeNonlinearInequalityConstrained(TestMinimizeBase):
    ...
    def test_simple(self, fun, n, x0, xl, xu, cub, x_best, fun_best):
        res = minimize(
            fun=getattr(self, fun),
            x0=x0,
            constraints=NonlinearConstraint(cub, -np.inf, 0.0),
            options={"debug": True},
        )
        self.assert_result(res, n, x_best, fun_best, 0,
                           np.sqrt(np.finfo(float).eps))
    ...
    def test_tough(self, fun, n, x0, xl, xu, cub_tough):
        minimize(
            fun=getattr(self, fun + "_tough"),
            x0=x0,
            constraints=NonlinearConstraint(cub_tough, -np.inf, 0.0),
        )
...
class TestMinimizeNonlinearEqualityConstrained(TestMinimizeBase):
    ...
    def test_simple(self, fun, n, x0, xl, xu, ceq, x_best, fun_best):
        res = minimize(
            fun=getattr(self, fun),
            x0=x0,
            constraints=NonlinearConstraint(ceq, 0.0, 0.0),
            options={"debug": True},
        )
        self.assert_result(res, n, x_best, fun_best, 0,
                           np.sqrt(np.finfo(float).eps))
    ...
    def test_tough(self, fun, n, x0, xl, xu, ceq_tough):
        minimize(
            fun=getattr(self, fun + "_tough"),
            x0=x0,
            constraints=NonlinearConstraint(ceq_tough, 0.0, 0.0),
        )
