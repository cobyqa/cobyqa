...
def drotg(double a, double b):
    ...
    cdef double c = c_ptr[0]
    cdef double s = s_ptr[0]
    free(<void *> c_ptr)
    free(<void *> s_ptr)
    return a_ptr[0], b_ptr[0], c, s

def drot(np.ndarray[float64] x, np.ndarray[float64] y, double c, double s):
    cdef int inc_x = x.strides[0] / x.itemsize
    cdef int inc_y = y.strides[0] / y.itemsize
    cblas.cblas_drot(x.size, <float64 *> x.data, inc_x, <float64 *> y.data, inc_y, c, s)

def dgeqp3(np.ndarray[float64, ndim=2] a, np.ndarray[lapack_int] jpvt, np.ndarray[float64] tau):
...
def dgeqrf(np.ndarray[float64, ndim=2] a, np.ndarray[float64] tau):
    cdef lapack_layout matrix_layout = get_lapack_layout(a)
    return clapack.LAPACKE_dgeqrf(matrix_layout.layout, a.shape[0], a.shape[1], <float64 *> a.data, matrix_layout.lda, <float64 *> tau.data)

def dorgqr(np.ndarray[float64, ndim=2] a, np.ndarray[float64] tau):
    cdef lapack_layout matrix_layout = get_lapack_layout(a)
