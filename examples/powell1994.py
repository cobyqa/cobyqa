...
def _constraints(no):
    ...
    if no in "ADE":
        ...
        constraints.append(
            NonlinearConstraint(
                lambda x: [
                    x[0] ** 2.0 + 2.0 * x[1] ** 2.0 + 3.0 * x[2] ** 2.0,
                ],
                -np.inf,
                [1.0],
            )
...
def _solution(no):
    if no == "A":
        ...
        s = ([2.330499, 1.951372, -0.4775414, 4.365726, -0.6244870, 1.038131,
              1.594227],)
    elif no == "J":
        s = (
            [
...
if __name__ == "__main__":
    ...
    print(
        "| Problem | Evaluations | Objective function "
        "| Constraint violation | Distance to solution |"
    )
    print(
        "+---------+-------------+--------------------"
        "+----------------------+----------------------+"
    ...
    for problem in "ABCDEFGHIJ":
        res = minimize(
            fun, _x0(problem), problem, _bounds(problem), _constraints(problem)
        )
        print(
            f"|   ({problem})   |{res.nfev:^13}|{res.fun:^20.4e}"
            f"|{_resid(res.x, problem):^22.4e}"
            f"|{_distance(res.x, problem):^22.4e}|"
        )
    print(
        "+---------+-------------+--------------------"
        "+----------------------+----------------------+"
